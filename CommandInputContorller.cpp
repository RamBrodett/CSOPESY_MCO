    ...
    if (ScreenManager::getInstance()->getCurrentScreen()->getName() == "main") {
        if (command == "initialize") {
            ...
            if (Scheduler::getInstance()->getSchedulerRunning()) {
                ...
                cout << "Creating 10 processes, each with 100 print commands...\n";
                for (int i = 0; i < 10; ++i) {
                    string screenName = "proc" + to_string(i);
                    vector<Instruction> instructions;
                    for (int j = 0; j < 100; ++j) {
                        string message = "Message from " + screenName + ", iteration " + to_string(j + 1);
                        instructions.push_back({ InstructionType::PRINT, {}, message });
                    }
